...
class PointNeuMF(nn.Module):
    ...
    def _init_weight_(self):
        ...
        if self.initializer == 'normal':
            ...
            initializer(self.embed_user_GMF.weight)
            initializer(self.embed_item_GMF.weight)
            initializer(self.embed_user_MLP.weight)
            initializer(self.embed_item_MLP.weight)
            for m in self.MLP_layers:
                if isinstance(m, nn.Linear):
                    initializer(m.weight)
            initializer(self.predict_layer.weight)
            for m in self.modules():
                if isinstance(m, nn.Linear) and m.bias is not None:
                    m.bias.data.zero_()
        # if not self.model == 'NeuMF-pre':
