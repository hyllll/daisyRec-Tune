...
class PointData(data.Dataset):
    def __init__(self, neg_set, is_training=True, neg_label_val=0):
        ...
        Parameters
        ----------
        neg_set : np.array, negative sampled result generated by Sampler
        ...
        pos_features = neg_set[:, [0, 1]]
        pos_features = np.unique(pos_features, axis=0)
        pos_label = np.ones(pos_features.shape[0], dtype=np.float32)
        if is_training:
            neg_features = neg_set[:, [0, 2]]
            neg_label = np.zeros(neg_features.shape[0], dtype=np.float32)
            neg_label[:] = neg_label_val
            self.features_fill = np.vstack((pos_features, neg_features))
            self.labels_fill = np.hstack((pos_label, neg_label))
        else:
            self.features_fill = pos_features
            self.labels_fill = pos_label
